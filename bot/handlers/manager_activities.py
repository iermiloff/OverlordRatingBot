...
async def cmd_manager_activities(message_or_query, is_manager: bool, db_session: AsyncSession):
    ...
    if not rewards:
        ...
        for idx, r in enumerate(rewards, start=1):
            type_label = "💎 Рейтинг" if str(r.reward_type) == "rating" else "🎒 Мерч"
    ...
    text = (
        ...
        f"_(появится в чате между {sys_settings.chest_quiet_hours} и {max_time} минутами)_\n\n"
...
async def process_add_reward_start(callback: CallbackQuery, is_manager: bool, state: FSMContext):
    ...
    await callback.message.answer(" **Настройка [Шаг 1/3]**\n\nВыбери тип награды:", reply_markup=get_reward_type_keyboard())
...
async def process_reward_type_choice(callback: CallbackQuery, state: FSMContext):
    # СТРОГО ИСПРАВЛЕНО: Берем элемент по индексу, чтобы получить чистую строку 'rating' или 'item'
    ...
    if chosen_type == "rating":
        prompt = f"Введите **количество {settings.CURRENCY_NAME}**, которое получит юзер (целое число):"
    else:
        prompt = "Введите **название физического мерча** (например: _Худи с логотипом_):"
        
    await callback.message.edit_text(f"🎁 **Настройка сундука [Шаг 2/3]**\n\n{prompt}", parse_mode="Markdown")
    await callback.answer()
...
async def process_set_timer_start(
    ...
    await callback.message.answer(
        ...
        "Введите количество **гарантированных МИНУТ тишины** (напр. `15`):"
    )
    await callback.answer()
...
async def process_quiet_hours_input(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Введите **диапазон рандома в МИНУТАХ** (например, `30`):"
    )
            ...
            try:
                await callback.bot.send_message(
                    ...
                    text=f"📦 **Сундук:** @{db_user.username or db_user.tg_id} "
                         f"выиграл мерч *{win_reward.value}*.",
                    parse_mode="Markdown"
                )
            except Exception: pass
    await callback.answer()
